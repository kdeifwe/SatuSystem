...
- 2026-08-14: Короткий build-breaking коммит `67dd4c5` (~10:43-10:45 +05:00) породил FAILED Railway deployment, но новый билд не был опубликован; старый рабочий деплой оставался активным, после фикса `de4e36d` сборка прошла и деплой/публикация восстановились без даунтайма.
...
Что осталось
- Граф знаний с типами связей — отложено осознанно; текущий similarity-auto-linking признан достаточным для текущего релиза.
- Conversation memory — оценить через неделю изменение доли `update_lead_info` среди `agent_response` вызовов и принять решение по дальнейшему хранению памяти.
- Чистка тестовых `fixture`-агентов в таблице `agents` (гигиена; не блокирует продовую работу).

Разное, не проверено
- (Перенесено) `kb_chunk_links` подробности перенесены в раздел «Что сделано» после выполнения refresh.

Сессия по улучшению KB search завершена 2026-08-14. Проверки, исправления и ручная миграция `kb_health_check()` применены; дальнейшие задачи перечислены в разделе "Что осталось".
