...
const email = `agent-smoke-${Date.now()}@mailinator.com`;
...
const { error: orgError } = await admin.from('organizations').insert({ id: orgId, name: `Smoke Org ${Date.now()}` });
...
const patchResult = await request(`/api/agents/${agentId}`, {
  ...
  body: JSON.stringify({ goal: 'patched-via-http' }),
});
